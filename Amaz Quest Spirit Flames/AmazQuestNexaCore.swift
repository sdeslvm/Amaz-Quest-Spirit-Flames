...
struct AmazQuestWebViewBox: UIViewControllerRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: AmazQuestWebLoader
    var defaultOrientations: UIInterfaceOrientationMask = .portrait

    // MARK: - Координатор

    func makeCoordinator() -> AmazQuestWebCoordinator {
        AmazQuestWebCoordinator { [weak loader] status in
    ...
    func makeUIViewController(context: Context) -> AmazQuestBaseWebViewController {
        let configuration = createWebViewConfiguration()
        return AmazQuestBaseWebViewController(
    ...
    func updateUIViewController(_ uiViewController: AmazQuestBaseWebViewController, context: Context) {
