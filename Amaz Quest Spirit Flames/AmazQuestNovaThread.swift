...
protocol AmazQuestLoadStateRepresentable {
    var type: AmazQuestLoadState.StateType { get }
...
struct AmazQuestLoadState: Equatable, AmazQuestLoadStateRepresentable {
    ...
    static func idle() -> AmazQuestLoadState {
        AmazQuestLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> AmazQuestLoadState {
        AmazQuestLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> AmazQuestLoadState {
        AmazQuestLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> AmazQuestLoadState {
        AmazQuestLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> AmazQuestLoadState {
        AmazQuestLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: AmazQuestLoadState) -> Bool {
    ...
    static func == (lhs: AmazQuestLoadState, rhs: AmazQuestLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension AmazQuestLoadState: CustomStringConvertible {
