...
struct AmazQuestGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://amazquestspirit.pw/image")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            AmazQuestEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}
